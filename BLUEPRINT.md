...
Business Process Analyst and operations-systems builder. AI-assisted workflows
are secondary technical work, not the primary professional identity. The site
presents a compact landing page, an evidence-led resume narrative, a focused
projects page, a direct-link technical writeup, and a one-click resume PDF. The
site itself doubles as the UI/UX work sample: hand-built static HTML/CSS/JS,
accessible, responsive, fast, and framework-free.
...
> the resume, a project, or a supporting artifact - with zero broken links and zero
> client-identifying information.

## Goals And Pillars

- **Evidence over feature lists:** the resume page connects process-analysis
  capabilities to specific controls, outcomes, and supporting projects.
- **The site is the work sample:** quality/UX craft is demonstrated in the
  artifact itself - responsive layouts, dark/light theme, keyboard navigation,
  reduced-motion support, and no gimmicks.
- **Anonymized but real:** verified outcomes and architectures are supported by
  publishable claims and anonymized or synthetic artifacts.
...
| Product surface | Four-page site: index, resume, projects, and OpenBrain writeup | one shared stylesheet and script; progressive enhancement, works with JS off |
...
- **Anonymization is absolute in portfolio content:** no client names, no
  employee names, no PHI, no real
  per-client financials. Generic descriptors only ("a seasonal wagering
  client"). Enforced by the verify-site leak scan.
- **Publishable evidence only.** Portfolio content uses anonymized claims and
...
The site files, resume assets, and verification checks remain implementation
truth.
...
- Not a game-dev portfolio; the D&D project appears only as a personal-project
  card and does not define the professional positioning.
...
| [S-011 - Vercel Portfolio Consolidation](specs/S-011-vercel-portfolio-consolidation/SPEC.md) | Consolidate the public portfolio on the existing `kayden-clark` Vercel project and retire the duplicate `resume-portfolio` project after production verification. | complete |
<!-- spec-catalog:end -->
